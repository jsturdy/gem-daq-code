...
std::string gem::hw::GEMHwDevice::getBoardID() const
{
...
std::string gem::hw::GEMHwDevice::getSystemID() const
{
  //LockGuard<Lock> guardedLock(lock_);
  std::string res = "???";
...
std::string gem::hw::GEMHwDevice::getFirmwareDate(std::string const& regNamePrefix) const
{
...
std::string gem::hw::GEMHwDevice::getFirmwareVer(std::string const& regNamePrefix) const
{
...
uint32_t gem::hw::GEMHwDevice::readReg(std::string const& name) const
{
  //LockGuard<Lock> guardedLock(lock_);
...
void gem::hw::GEMHwDevice::readRegs(std::vector<std::pair<std::string, uint32_t> > &regList) const
{
  //LockGuard<Lock> guardedLock(lock_);
  uhal::HwInterface& hw = getGEMHwInterface();
...
void gem::hw::GEMHwDevice::writeRegs(std::vector<std::pair<std::string, uint32_t> > const& regList) const
{
  //LockGuard<Lock> guardedLock(lock_);
  uhal::HwInterface& hw = getGEMHwInterface();
  int retryCount = 0;
...
void gem::hw::GEMHwDevice::writeValueToRegs(std::vector<std::string> const& regNames, uint32_t const& regValue) const
{
...
void gem::hw::GEMHwDevice::zeroReg(std::string const& name) const
{
  writeReg(name,0);
}

void gem::hw::GEMHwDevice::zeroRegs(std::vector<std::string> const& regNames) const
{
...
std::vector<uint32_t> gem::hw::GEMHwDevice::readBlock(std::string const& name) const
{
  //LockGuard<Lock> guardedLock(lock_);
  uhal::HwInterface& hw = getGEMHwInterface();
...
//void gem::hw::GEMHwDevice::zeroBlock(std::string const& name) const
//{
//  //LockGuard<Lock> guardedLock(lock_);
//  uhal::HwInterface& hw = getGEMHwInterface();
//  size_t numWords = hw.getNode(name).getSize();
...
void gem::hw::vfat::HwVFAT2::writeReg(std::string const& regName
				      uint32_t const writeValue) const
{
  //Maybe want to use a lock to prevent hammering the HW device
  uhal::HwInterface& hw = getGEMHwInterface();
  try {
