...
{
  /*
  DEBUG("OptoHybridManager::init begin");
  for (int slot = 0; slot < MAX_AMCS_PER_CRATE; ++slot) {
    ...
    for (int link = 0; link < MAX_OPTOHYBRIDS_PER_AMC; ++link) {
      ...
      DEBUG("OptoHybridManager::creating hwCfgInfoSpace items for board connected on link " << link << " to GLIB in slot " << (slot+1));
      ...
      if (xdata::getInfoSpaceFactory()->hasItem(hwCfgURN.toString())) {
        DEBUG("OptoHybridManager::init::infospace " << hwCfgURN.toString() << " already exists, getting");
        is_optohybrids[slot] = xdata::getInfoSpaceFactory()->get(hwCfgURN.toString());
      } else {
        DEBUG("OptoHybridManager::init::infospace " << hwCfgURN.toString() << " does not exist, creating");
      ...
      DEBUG("OptoHybridManager::InfoSpace found item: ControlHubAddress " << is_optohybrids[slot]->find("ControlHubAddress"));
      DEBUG("OptoHybridManager::InfoSpace found item: IPBusProtocol "     << is_optohybrids[slot]->find("IPBusProtocol")    );
      DEBUG("OptoHybridManager::InfoSpace found item: DeviceIPAddress "   << is_optohybrids[slot]->find("DeviceIPAddress")  );
      DEBUG("OptoHybridManager::InfoSpace found item: AddressTable "      << is_optohybrids[slot]->find("AddressTable")     );
      DEBUG("OptoHybridManager::InfoSpace found item: ControlHubPort "    << is_optohybrids[slot]->find("ControlHubPort")   );
      DEBUG("OptoHybridManager::InfoSpace found item: IPBusPort "         << is_optohybrids[slot]->find("IPBusPort")        );
      
      DEBUG("OptoHybridManager::info:" << info.toString());
