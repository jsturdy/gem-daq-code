...
namespace gem {
  namespace utils {
    ...
    public:
       ...
       */
      enum ItemType { HW8,      //!< Used for VFAT registers, 8-bits only
      ...
      {
      public:
      GEMInfoSpaceItem(std::string const& itemname, UpdateType itype, UpdateType utype, std::string const& docstring) :
        m_name(itemname), m_itype(itype), m_utype(utype), m_docstring(docstring) {};
        virtual ~GEMInfoSpaceItem() {};
        std::string m_name;
        ItemType    m_itype;
        UpdateType  m_utype;
    ...
    private:
      std::unordered_map<std::string, std::pair<std::string, GEMInfoSpaceItem& > >  m_itemMap;
